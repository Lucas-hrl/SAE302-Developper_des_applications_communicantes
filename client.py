...
class ClientWindow(QMainWindow):
    ...
    signal_maj_routeurs = pyqtSignal(int)
    ...
    def __init__(self):
        ...
        self.input_port_dest = QLineEdit("")
    ...
    def action_demarrer(self):
        ...
        try:
            ...
            t = threading.Thread(target=self.thread_ecoute, daemon=True)
            t.start()
    ...
    def action_verifier_routeurs(self):
        ...
        try:
            ...
            t = threading.Thread(target=self.thread_recuperer_info_master, args=(master_ip, master_port))
            t.start()
        except ValueError:
            self.signal_log.emit("Erreur: Port Master invalide")

    def thread_recuperer_info_master(self, ip, port):
        try:
            routeurs = self.obtenir_liste_routeurs(ip, port)
            nb = len(routeurs)
            self.signal_log.emit(f"Réponse Master: {nb} routeur(s).")
            self.signal_maj_routeurs.emit(nb)
        except (OSError, ValueError) as e:
            self.signal_log.emit(f"Erreur connexion Master: {e}")
            self.signal_maj_routeurs.emit(0)

    def obtenir_liste_routeurs(self, ip_master, port_master):
    ...
    def lancer_thread_envoi(self):
        ...
        try:
            ...
            t = threading.Thread(target=self.processus_envoi,
                             args=(msg, dest_ip, dest_port, nb_sauts, master_ip, master_port))
            t.start()
        except ValueError:
