import socket
import threading
import random
from crypt import CryptoSym


class Client:
    def __init__(self, host: str, port: int):
        self.host = host
        self.port = port
        self.clientsocketecoute = None  # Pour garder une trace du socket d'écoute
        self.running = False  # Pour pouvoir arrêter proprement le thread

    @property
    def host(self):
        return self._host

    @property
    def port(self):
        return self._port

    @host.setter
    def host(self, host: str):
        self._host = host

    @port.setter
    def port(self, port: int):
        self._port = port

    def __str__(self):
        return f"L'adresse ip du client est {self.host}, il écoute sur le port {self.port}"

    def envoyermessage(self, ipdist: str, portdist: int, message) -> bool:
        """
        Se connecte à un serveur distant et envoie un message.
        'message' peut être un str ou des bytes.
        """
        try:
            clientsocketenvoi = socket.socket()
            clientsocketenvoi.connect((ipdist, portdist))
            print(f"Connecté avec succès à {ipdist}:{portdist}")

            # Si c'est des bytes, on envoie directement, sinon on encode
            if isinstance(message, bytes):
                clientsocketenvoi.sendall(message)
            else:
                clientsocketenvoi.send(message.encode("latin1"))

            print("Message envoyé")
            clientsocketenvoi.close()
            return True
        except ConnectionRefusedError:
            print(f"Impossible de se connecter à {ipdist}:{portdist}")
            return False

    def demarrerecoute(self):
        """Lance le thread d'écoute pour ne pas bloquer le programme"""
        self.running = True
        thread = threading.Thread(target=self.boucleecoute)
        thread.start()

    def boucleecoute(self):
        """Méthode interne qui tourne en boucle pour accepter les connexions"""
        try:
            self.clientsocketecoute = socket.socket()
            self.clientsocketecoute.bind((self.host, self.port))
            self.clientsocketecoute.listen(2)  # 2 est le nombre de connexions en attente max
            print(f"Écoute démarrée sur {self.host}:{self.port}...")

            while self.running:
                conn, addr = self.clientsocketecoute.accept()
                print(f"Connexion acceptée de {addr}")
                donnee = conn.recv(1024)
                if donnee:
                    messagerecu = donnee.decode("latin1", errors="ignore")
                    print(f"Message reçu : {messagerecu}")
                    print("Appuyez sur Entrée pour continuer...")
                conn.close()
        except Exception as e:
            print(f"Erreur lors de la mise en écoute : {e}")


def getip():
    """Récupère l'IP locale"""
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # AF_INET car ipv4 et SOCK_DGRAM car UDP
    try:
        # On crée un socket pour tester la route vers internet
        s.connect(("8.8.8.8", 1))
        ip = s.getsockname()[0]
    except:
        ip = "127.0.0.1"
    finally:
        s.close()
    return ip


def recupererlisterouteurs(ipmaster, portmaster):
    try:
        s = socket.socket()
        s.connect((ipmaster, portmaster))
        s.send(b"LIST")
        data = s.recv(4096).decode("latin1")
        print("DEBUG LIST reçu du Master :", repr(data))

        s.close()

        if data == "" or data == "vide":
            return []

        listepropre = []
        for item in data.split(","):
            ip, port, cle = item.split(":", 2)
            listepropre.append((ip, int(port), cle))
        return listepropre
    except Exception as e:
        print(f"Erreur connexion Master : {e}")
        return []



def construire_oignon(message_final: str, ip_dest: str, port_dest: int, circuit: list) -> bytes:
    """
    Construit le paquet "oignon" en chiffrant couche par couche.

    circuit : liste de tuples (ip, port, cle) des routeurs à traverser, dans l'ordre.
    Retourne des bytes chiffrés à envoyer au premier routeur.

    Format attendu par CHAQUE routeur après déchiffrement :
      - "ip_suivante;port_suivant;reste"  -> relais
      - ";;message_final"                 -> destinataire final
    """

    # 1) Couche la plus interne : ce que verra le DERNIER routeur
    # Il doit envoyer au client final, donc après déchiffrement par le dernier routeur,
    # on veut directement le message final sans avoir à relayer.
    # On encode donc : ";;message_final"
    paquet = f";;{message_final}"

    # 2) Construire les couches en partant du DERNIER routeur vers le PREMIER
    # circuit = [R1, R2, ..., Rn]
    # On part de Rn, puis Rn-1, ..., R1
    for i in range(len(circuit) - 1, -1, -1):
        ip_routeur, port_routeur, cle_routeur = circuit[i]

        # Pour ce routeur, on doit mettre dans le texte clair :
        # "ip_suivante;port_suivante;reste"
        # - si c'est le DERNIER routeur du circuit :
        #       ip_suivante = ip_dest
        #       port_suivante = port_dest
        # - sinon :
        #       ip_suivante = IP du routeur suivant dans le circuit
        #       port_suivante = port du routeur suivant

        if i == len(circuit) - 1:
            # Dernier routeur : il enverra au destinataire final
            ip_suivante = ip_dest
            port_suivante = port_dest
        else:
            # Routeur intermédiaire : il enverra vers le routeur suivant
            ip_suivante = circuit[i + 1][0]
            port_suivante = circuit[i + 1][1]

        # Construire le texte clair pour ce routeur
        # "ip_suivante;port_suivante;paquet_actuel"
        texte_clair = f"{ip_suivante};{port_suivante};{paquet}"

        # Chiffrer ce texte clair avec la clé de CE routeur
        crypto = CryptoSym(cle=cle_routeur)
        paquet_chiffre = crypto.chiffrer(texte_clair)

        # Ce paquet chiffré devient le "paquet" pour la couche supérieure
        paquet = paquet_chiffre.decode("latin1")

    # À la fin de la boucle, "paquet" est une str latin1, chiffrée pour le PREMIER routeur
    # On la retransforme en bytes pour l'envoi
    return paquet.encode("latin1")



if __name__ == "__main__":
    moi = Client(getip(), 5000)
    moi.demarrerecoute()

    ipmaster = input("IP du master : ")
    portmaster = 9016

    while True:
        print("\n--- NOUVEAU MESSAGE ---")

        dest = input("IP Destinataire : ")
        msg = input("Message : ")

        # Récupérer la liste des routeurs avec leurs clés
        routeursdispos = recupererlisterouteurs(ipmaster, portmaster)

        if not routeursdispos:
            print("Erreur : Aucun routeur disponible via le Master !")
            continue

        # Afficher le nombre de routeurs en ligne
        maxdispo = len(routeursdispos)
        print(f"{maxdispo} routeur(s) disponible(s) en ligne")

        # Demander par combien de routeurs passer
        try:
            nb = int(input(f"Par combien de routeurs passer ? (max {maxdispo}) : "))
            # Sécurité : on borne le chiffre entre 1 et le max disponible
            if nb > maxdispo:
                nb = maxdispo
            if nb < 1:
                nb = 1
        except:
            nb = 1  # Par défaut si l'utilisateur tape n'importe quoi

        # Sélection aléatoire des routeurs pour le circuit
        circuit = random.sample(routeursdispos, nb)

        # Affichage du chemin
        cheminstr = " -> ".join(r[0] for r in circuit)
        print(f"Circuit généré : Moi -> {cheminstr} -> Destinataire")

        # Construction de l'oignon (chiffrement multicouche)
        paquet = construire_oignon(msg, dest, 5000, circuit)

        # Envoi au premier routeur
        premier = circuit[0]
        moi.envoyermessage(premier[0], premier[1], paquet)
