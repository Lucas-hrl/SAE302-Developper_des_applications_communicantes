...
class Master:
    ...
    def _boucle_ecoute(self):
        ...
        try:
            ...
            self.socket_ecoute.listen(10)  # File d'attente
            print(f"Master en écoute sur {self.hote}:{self.port}")
            self._log("DEMARRAGE_MASTER", self.hote, f"port={self.port}")
            ...
            while self.en_cours:
                try:
                    ...
                    # À chaque client, on lance un thread dédié pour traiter sa demande
                    t = threading.Thread(target=self._traiter_demande, args=(conn, addr))
                    t.start()
                except OSError:
            ...
            self._log("ERREUR_MASTER", self.hote, f"{e}")
        finally:
